...
query_list = generate_query_list('csv//Team Sheet - Enterprise Stockmaster - Inventory.csv', 'LAPTOP', 'APPLE')
print(query_list)
...
def get_ID_sold_date(url, cookies, search):
    ...
    df.to_csv(f'csv/SoldDates/Enterprise/{search}_output_SoldDate.csv', index=False)
...
def main(num_queries=None, num_results=None, num_products=None, randomize_queries=True, query_fraction=None):
    ...
    for query in tqdm(queries_to_scrape, desc="Processing queries", unit="query"):
        ...
        df.to_csv(f'csv/ProductData/Enterprise/{search}_output_.csv', index=False)
        #display(df)
...
folder_path = 'csv//ProductData//Enterprise'
...
os.chdir("csv//SoldDates//Enterprise")
extension = 'csv'
...
#export to csv
combined_csv.to_csv('temp//sold_dates_combined_enterprise.csv', index=False, encoding='utf-8-sig')

# Read the data from the CSV file
sold_dates_combined = pd.read_csv('temp//sold_dates_combined_enterprise.csv')
...
folder_path = 'ProductData//Enterprise'
...
# Write the DataFrame to a CSV file and display it
df2.to_csv(f'temp//AlignedComplete_enterprise.csv', index=False)
...
# Write the DataFrame to a CSV file and display it
df2.to_csv(f'dataset//update//update_enterprise_pre_clean.csv', index=False)
