...
def search_nested_classes(soup, search_words, lnln):
...
def main(num_queries=1, num_results=1, num_products=5):
    ...
    queries_to_scrape = query_list if num_queries is None else query_list[:num_queries]
    ...
    for query in tqdm(queries_to_scrape, desc="Processing queries", unit="query"):
        ...
        for page in tqdm(results_to_scrape, desc="Processing pages", unit="page"):
            ...
            for lnln in tqdm(products_to_process, desc="Processing products", unit="product"):
                ...
                # Initialize item_specifics before the if statement
                item_specifics = None
                ...
                if response.ok:
                    ...
                    soup = BeautifulSoup(response.text, 'lxml')
                    # Extract the specific data from the HTML content
                    item_specifics = search_nested_classes(soup, search_words, lnln)
                else:
...
# Write the DataFrame to a CSV file and display it
df2.to_csv(f'dataset//update//update.csv', index=False)
