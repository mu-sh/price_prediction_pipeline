...
#df = pd.read_csv('csv//temp//AlignedComplete.csv')
...
storage_map = {'emmc': 'emmc', 'm.2 ssd': 'm.2', 'm.2 drive': 'm.2',  'm2' : 'm.2', 'm2 chip' : 'm.2', 'nvme': 'nvme', 'ssd nvme': 'nvme', 'sshd (solid state hybrid drive)': 'sshd', 'sshd': 'sshd', 'ssd (solid state drive)': 'ssd', 'ssd': 'ssd', 'solid state drive' : 'ssd', 'hdd (hard disk drive)': 'hdd', 'hdd': 'hdd'}
...
df2['model'] = df2.apply(lambda row: str(row['model']).replace(str(row['brand']), '').strip(), axis=1)
...
#df.to_csv(f'dataset//base//base_TEST.csv', encoding='utf-8', index=False)
